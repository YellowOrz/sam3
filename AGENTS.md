...
- `qualitative_test_interactive.py`：在桌面窗口中逐帧检查、修正并双向传播实例掩码，输出无损掩码、结果视频和元数据。运行：`python scripts/qualitative_test_interactive.py --video INPUT.mp4 --output-dir OUT --device cuda:0`；批量运行用 `--input-root DATA --output-root OUT --rgb-name color.mp4`，递归排序并逐个打开，输出去掉末尾 `raw` 层再追加 `--output-subdir`（默认 `masks_sam3`）和提示词目录（空格替换为下划线，如 `left hand` → `left_hand`），其他层级保留；单视频输出不受影响。整批复用一个模型，每视频/分块独立 session。批量跳过匹配的完整成功结果，不完整或未确认结果自动重跑；完整但设置不匹配的结果及单视频已有输出仍需 `--overwrite`；失败继续并在末尾汇总，关闭编辑窗口或 Ctrl+C 停止整批，分块仍需终端 `ok` 确认。需图形环境与 CUDA。
