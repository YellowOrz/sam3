...
共用代码位于 `scripts/common/`：`video_utils.py` 提供视频工具，`compare_gt_masks.py` 提供 GT 参数、评测和批次汇总。`process_learned_prompt_videos.py`、`process_mano_prompt_videos.py`、`process_dataset_videos.py` 统一通过 `--compare-gt --gt-mask-name NAME` 启用评测；`--gt-dir-name` 默认 `masks_sam3`，`--compare-skip-frames` 默认 `0`，GT 文件名不从提示词、目标 ID 或手侧推导。
