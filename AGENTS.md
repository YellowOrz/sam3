...
- `qualitative_test_interactive.py`：在桌面窗口中逐帧检查、修正并双向传播实例掩码，输出无损掩码、结果视频和元数据。运行：`python scripts/qualitative_test_interactive.py --video INPUT.mp4 --output-dir OUT --device cuda:0`；批量运行用 `--input-root DATA --output-root OUT --rgb-name color.mp4`，递归排序并逐个打开，整批复用一个模型，每视频/分块独立 session。批量跳过匹配的完整成功结果，覆盖需 `--overwrite`；失败继续并在末尾汇总，关闭编辑窗口或 Ctrl+C 停止整批，分块仍需终端 `ok` 确认。需图形环境与 CUDA。
