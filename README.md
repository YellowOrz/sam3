...
Add `--detector-only` to skip tracker propagation, association, and memory updates, including on the initial prompt frame. The main `masks.mkv` then stores the per-frame detector union (background 0, foreground 1), with no persistent instance IDs; `result.mp4` and GT evaluation use this same union. Missing geometry falls back to text detection; frames with neither text nor geometry are empty. `--save-detector` remains supported and writes the same mask content as an additional detector output. The full model weights are still loaded. Forward/backward modes retain their output ordering, but no temporal information is used. Metadata distinguishes detector-only and tracked predictions, so changing the mode reruns inference; use separate output directories to retain both experiments.
...
Videos are discovered recursively in relative-path order. Output paths remove a trailing `raw` directory and append `--output-subdir` (default: `masks_sam3`) followed by the text prompt with each space replaced by an underscore, preserving other directory levels. For example, `--output-subdir masks_sam3 --text-prompt "left hand"` maps `SESSION/SEGMENT/raw/rgb.mkv` to `OUTPUT_ROOT/SESSION/SEGMENT/masks_sam3/left_hand/`. This option does not affect single-video `--output-dir`. The input and output roots must differ. Each video or chunk gets its own session. Press `Q` after all frames have been processed to save and advance; chunked videos additionally require terminal `ok` confirmation. Close the editor or press `Ctrl+C` to stop the batch. Closing the final review playback still returns to its terminal prompt.

Complete successful outputs matching the input, prompt, model version, and chunk setting are skipped. Incomplete or unconfirmed batch outputs are automatically reprocessed without `--overwrite`. Complete outputs with mismatched settings still require `--overwrite`. Individual failures do not stop the batch; their paths and errors are reported at the end with a nonzero exit code. Saved files are retained on interruption; in-memory tracker checkpoints do not support restart recovery. The original `--video INPUT --output-dir OUT` mode remains available and requires `--overwrite` for existing outputs.
