#!/usr/bin/env python3
"""Unified VE / spatial / output-residual completed-epoch evaluation.
...
def require_residual_epoch(state, epoch):
    step = state['progress']['global_step']
    if (type(epoch) is not int or epoch < 1
            or step != epoch * state['training_config']['steps_per_epoch']
            or state.get('validation_state', {}).get('last_validation_step') != step):
        raise ValueError('Require explicit completed AND validated residual epoch')
...
def protocol(images, indices, base_hash, tokenizer_hash, annotation_hash, batch_size):
    return dict(format='sam3-hand-routes-v1', image_ids=[images[i]['id'] for i in indices],
        ...
        selection='highest confidence, never GT matching', boundary_pixels=4,
        reference_policy='per-side flags excluded from primary; missing is unknown',
...
def main():
    ...
    if (args.method == 've' and any((args.adapter_checkpoint, args.delta_checkpoint, args.epoch,
        ...
        parser.error('VE takes no checkpoint; spatial requires adapter+epoch; residual requires delta+epoch+cache+training root')
    ...
    if args.method == 'residual':
        for path in (args.delta_checkpoint, args.initial_cache, args.training_data_root / 'annotations.json'):
            hashes[str(path.resolve())] = full.shared.sha256(path)
        require_residual_epoch(torch.load(args.delta_checkpoint, map_location='cpu', weights_only=True), args.epoch)
        encoder, metadata = full.fixed.load_residual(args.delta_checkpoint, args.initial_cache,
            args.training_data_root, base_hash=full.shared.sha256(args.base_checkpoint),
            tokenizer_hash=full.shared.sha256(args.tokenizer_path))
    ...
    selected_path = args.adapter_checkpoint or args.delta_checkpoint
    summary = dict(status='running', method=args.method,
        ...
        checkpoint_sha256=full.shared.sha256(selected_path) if selected_path else None,
        residual_metadata=metadata,
        protocol=protocol(images, indices, full.shared.sha256(args.base_checkpoint),
            full.shared.sha256(args.tokenizer_path), full.shared.sha256(root / 'annotations.json'), args.batch_size),
        note='SAM3-assisted reference, not independent manual ground truth; no model selection')
    summary['protocol'].update(publication_sha256=publication_hash,
        implementation_sha256=hashlib.sha256(json.dumps({str(Path(p).relative_to(project)): digest
            for p, digest in sorted(implementation.items())}, sort_keys=True).encode()).hexdigest())
    ...
    try:
        ...
        if encoder is not None:
            full.cached.install_cached_ve_text_encoder(model, encoder.to('cuda'))
            full.cached.set_cached_ve_training_mode(model, train_delta=False)
        cache_hash = full.initializer.cache_fingerprint(encoder.state_dict()) if encoder is not None else None
        ...
        records, visuals = full.evaluate(model, 've-both' if args.method == 've' else args.method, dataset,
        ...
        if encoder is not None and full.initializer.cache_fingerprint(encoder.state_dict()) != cache_hash:
