...
def parse_args(argv=None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--num-tokens", type=int, help="Total valid tokens for random init (1-32)"
    )
    ...
    if not math.isfinite(args.random_std) or args.random_std <= 0:
        parser.error("random-std must be finite and positive")
    if args.init == "text":
        if not args.reference_text or not args.reference_text.strip():
            parser.error("--init text requires nonempty --reference-text")
        if args.num_tokens is not None:
            parser.error("--num-tokens is only supported with --init random")
    else:
        if args.num_tokens is None or not 1 <= args.num_tokens <= 32:
            parser.error("--init random requires --num-tokens between 1 and 32")
        if args.reference_text is not None:
            parser.error("--init random uses --num-tokens, not --reference-text")
...
def main(argv=None) -> None:
    ...
    if args.init == "random":
        ...
        features = torch.randn((32, 256), generator=generator) * args.random_std
        mask = torch.arange(32) >= args.num_tokens
    else:
