...
def save_training_checkpoint(
    ...
    state = {
        "format": "sam3-learnable-class-tokens-v1",
...
def main() -> None:
    ...
    if not (args.data_root / "annotations.json").is_file():
        raise FileNotFoundError(args.data_root / "annotations.json")

    args.output_dir.mkdir(parents=True, exist_ok=True)
    latest_path = args.output_dir / f"k{args.tokens_per_class}_epoch1_latest.pt"
    final_path = args.output_dir / f"k{args.tokens_per_class}_epoch1_final.pt"
    summary_path = args.output_dir / f"k{args.tokens_per_class}_epoch1_summary.json"
    ...
    epoch_order = build_epoch_order(len(dataset), args.seed)
    start_step = 0
    loss_history: list[float] = []
    initial_left_tokens = token_encoder.class_tokens[0].detach().cpu().clone()
    ...
    if args.resume is not None:
        ...
        initial_left_tokens = resume_state["initial_left_tokens"].clone()
    else:
        print("3. K=4 token 从头随机初始化", flush=True)

    total_steps = len(epoch_order)
    ...
    try:
        for step in range(start_step, total_steps):
            dataset_index = epoch_order[step]
            sample = dataset[dataset_index]
            batch = collate_fn_api(
                [sample], dict_key="train", with_seg_masks=True
            )["train"]
            ...
            if float(gradient[0].abs().max().detach().cpu()) != 0.0:
                raise RuntimeError(f"step {step + 1}: left_hand token 获得了梯度")
            optimizer.step()
            ...
            if step == start_step or completed_steps % args.log_every == 0:
                ...
                print(
                    ...
                    f"sample_index={dataset_index} loss={loss_value:.6f} "
                    f"rolling={sum(recent) / len(recent):.6f} "
                    f"mask={float(losses['loss_mask'].detach().cpu()):.6f} "
                    f"dice={float(losses['loss_dice'].detach().cpu()):.6f} "
                    f"right_grad={float(gradient[1].norm().detach().cpu()):.6e}",
                    flush=True,
                )
            ...
            del sample, batch, output, prediction, targets, losses, loss
    ...
    left_change = float(
        (token_encoder.class_tokens[0].detach().cpu() - initial_left_tokens)
        .abs()
        .max()
    )
    if left_change != 0.0:
        raise RuntimeError(f"left_hand token 被意外修改: max_change={left_change}")
    ...
    summary = {
        "experiment": f"DexYCB right-hand K={args.tokens_per_class} epoch 1",
        ...
        "left_token_max_change": left_change,
    ...
    print("left_hand token 最大变化:", left_change, flush=True)
