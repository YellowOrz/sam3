...
## 可回退的 Tracker 纠正一致性实验（2026-09-17）

- 新增默认关闭的 `--consistent-reconditioning` 评估选项；不编辑冻结SAM3源码、不改权重、检测/关联/退出阈值，不自动升级SAM3.1，旧调用和输出格式保留。
- 仅在原生add-mask及preflight成功后，将对应检测选择到共享传播mask中，再执行原生遮挡处理与memory更新；当前输出复用同一来源，不用未抑制检测二次覆盖。不成功的纠正不登记，异常退出恢复实例级包装。
- 实验限定非multiplex、单GPU、非重入正向会话；可按完整录像独立分卡。记录策略SHA、成功ID及纠正/memory/输出来源指纹，不将新算法伪称存储修复等价。
- 增加成功/拒绝/异常、对象顺序、分辨率、遮挡后不复活、旧默认与上下文恢复的CPU测试，并保留完整录像回归及单变量评估门禁。工程验收与性能收益分别报告，文档和实验数据留仓库外。

## 完整录像分进程补齐与配对复算（2026-09-17）

- 视频评估新增明确命名的单录像执行选项，可在独立进程中从原始第0帧完整运行，避免跨录像保留CPU内存；禁止与截短工程模式混用，不切分单段时序，不改变模型、提示、阈值或采样锚点。
- 执行元数据记录所选录像，继续保留全计划SHA、逐帧旧前缀严格等价、CPU存储遥测及12GiB主机内存停止条件。原默认全计划执行行为不变。
- 增加不存在/歧义录像和截短组合的拒绝测试；补齐结果必须按原全计划核验覆盖，不将部分运行标为全量。

## 长视频评估的显存修复与严格等价验收（2026-09-16）

- 新增显式选择的纯前向 CPU 状态存储：为自动创建 tracker 的路径补齐原生 offload 参数，并将辅助逐帧输出缓存独立复制到 CPU；不清除历史、不切段、不改分辨率、阈值或模型参数。实例级包装可恢复，交互缓存读取明确拒绝，默认推理不变。
- 修复模式记录实际 mask/memory/cache 所在设备、去重存储量、CUDA allocated/reserved、主机内存；大持久张量未真实卸载或主机可用内存低于12GiB则停止。CPU复算还验证遥测覆盖与哈希，不仅检查旗标。
- 支持完整首段工程测试及旧结果的严格逐帧对照，mask RLE、ID、分数、几何任一差异即停止；保留失败现场，后续帧不冒充已有基线。新旧数据、权重、源码、设备与环境必须匹配。
- 新增 CPU 存储/过滤/异常恢复、共享存储去重、旧前缀覆盖与篡改拒绝测试。工程成功不替代长序列验收，实验与私人运行文档仍在仓库外。

## nakehand MANO 四条件外部评估（2026-09-16）

- 新增评估数据准备入口，复用已审计的连续 RGB 与双侧辅助参考，逐文件 SHA 校验；不重新编码图像、不训练、不用参考外接框替代 MANO 网格框。
- 支持同侧多个 MANO 文件按唯一活动源帧合并；活动帧重叠或孤立几何直接拒绝，不按参考质量择框。绑定投影代码、模型、权重与输入哈希。
- 独立 nakehand 协议保留原始帧号每三帧评分；显式排除帧仅保留视频 RGB 上下文，禁止伪造负参考。原 RealSense 协议不变。
- 同时记录跟踪前 Detector 与最终视频输出，完整 CPU 复算覆盖与排除帧；新增合并歧义、采样锚点、路径安全、参考排除和旧协议兼容测试。数据、实验文档及个人路径不入仓库。

## MANO 视频实例生命周期复现诊断（2026-09-16）

- 新增冻结输入/源码/权重的完整视频事件采集器，对照直接预测器和生产工厂入口；不改阈值、模型参数、关联结果或旧预测。
- 只读记录逐帧检测、关联IoU、跟踪实例、热启动删除、keep-alive、确认与输出隐藏，保留完整实例RLE，支持独立逐像素复算和重复运行检查。
- 增加无副作用快照、序列化隔离、关联IoU和原始返回值保留测试；实验报告、真实数据与个人环境路径仍留仓库外。

## MANO 框的 Detector／视频四条件诊断（2026-09-15）

- 新增独立评估入口，固定原始模型/源码/完整帧与 MANO 框，旁路读取同一 Detector 经 NMS/门控、跟踪前的结果与最终视频输出，避免不同图像 API 预处理混杂；不训练、不修改冻结模型源码。
- 原视频连续推理，固定零基 0、3、6…评分；缺失几何回退文字，缺参考不伪造负例。保存完整实例 RLE、分数、视频 ID 与实际接收框，校验逐帧几何和连续覆盖；视频 joint score 已含 presence，不再重复相乘。
- 增加独立 CPU 像素复算、四组配对汇总、逐视频/有无几何分组及预固定分离可视化。明确 frame 是视频内部无跟踪检测基线，不是独立图像 API；后续系统差异不能直接归因于 memory，辅助参考不当独立精标。
- 覆盖采样锚点、框格式、空/未知参考、所有实例并集、已接受输出不二次过滤、完整覆盖、边界/对侧重叠及完整 CPU 汇总与篡改拒绝测试。既有未提交的视频评估修改不纳入本次改动。

## 连续视频传播与逐三帧评分（2026-09-14）

- 视频评估新增显式评分stride：所有原始帧连续向前传播，仅对零基0、3、6…评分；不将采样后的帧送入memory。旧逐帧评分默认值保留。
- 同时报告学长口径的已提供参考、双方为空计1的逐帧mean Dice与pixel Dice，以及排除已知问题的敏感性、正参考Dice/边界、漏检和误报；未知参考不作负样本，左右及逐录像分开统计。
- 新增显式已完整验证的诊断step选择，允许评估600步短试而不伪称完整epoch；原epoch验收及训练数据/base/cache/恢复状态严格检查保持。
- 增加完整连续覆盖、双侧身份、采样锚点、空/未知/问题参考、像素与Dice一致性及诊断点拒绝测试。不修改已冻结实验或训练参数。

## 32位置解除屏蔽残差短程对照（2026-09-14）

- 新增显式 `unmasked32_delta` / `--residual-positions unmasked32`：保留原VE缓存与自然提示，输出32个有效位置并学习16384个残差参数；原 `all` 仍为四有效位置，默认行为不变。
- 配置/恢复严格绑定全32位置mask策略，并声明零delta不等价原VE；原padding缓存仍表示四个自然有效位置，不篡改来源。初态只允许原冻结/零残差缓存，禁止旧训练权重冒充新起点。
- 增加全部位置梯度、缓存不变、池化起点变化、策略错配拒绝及保存恢复一致性测试；统一评估加载器支持新模式，不更改检测阈值或旧协议。
- 新增Dex-only数据发布入口：从已批准混合数据筛原Dex train，逐图校验RGB哈希、保持原图像/标注记录和完整Dex val，不训练nakehand/RealSense，不覆盖源数据。
- 本模式用于同数据同预算的短程对照，必须分别报告解除屏蔽的未训练起点和训练后效果，不预设参数更多必然更好。

## MANO geometry 全时段 stride3 扩展（2026-09-13）

- 新增独立准备／推理／CPU复算入口，固定每段零基0、3、6…，保留空、未知、遮挡时段，不修改旧12帧pilot或训练快照。核验发布数据SHA、原始来源、现代MANO帧/侧/参考紧框对应；旧格式不猜投影。
- 点与网格框独立判可用；四个主要策略在缺失几何时显式回退同侧原文字，控制组缺失条件记未执行，禁止伪造负mask。保留原生Detector、冻结VE、最高分候选、严格score>0.5及完整geometry运行形状核验。
- 独立GPU整图分片和完整RLE/分数留存；CPU重新解码未变更参考PNG复算Dice/边界/漏检/误报，拒绝重叠或不完整分片。输出全时段、共同几何子集、左右、逐视频及参考面积分组与分栏图。
- 明确SAM3辅助参考和参考裁剪MANO的oracle限制，原图／参考／预测分开，未知与未执行用灰色；新增采样、点框独立性、回退、未知/空参考、严格阈值、投影、分片与独立评分测试。

## 末轮残差诊断（2026-09-13）

- CPU诊断增加显式训练点基线选项，默认仍要求原VE起点，拒绝倒序、相同步数及非法步数，不修改训练或评估快照。
- 独立从全部验证RLE重算4px边界，补充实际边界的门控／候选形状描述分解、左右漏检、负参考误报转移和概率统计；不把描述分解当因果证据。
- 新增有序比较、非法边界、空参考、负样本转移和Dice／边界加和恒等式测试。

## 正文位置输出残差对照（2026-09-13）

- 新增显式 `content_delta`：仅训练自然手提示中两个正文位置的1024参数，保留原SOT/EOT、完整32位置缓存、四个有效文本位置、padding及raw embeddings；原四位置2048参数模式默认不变。
- DDP入口增加 `--residual-positions all|content`，严格绑定模式、形状、参数量与初始零delta；保存/恢复、rank一致性及AdamW矩检查按声明布局校验，拒绝旧训练后权重冒充新方法初态。
- 统一图像/视频评估加载器支持新模式，仍核验训练数据、基础模型、初始化cache与恢复状态；旧单卡pilot协议不放宽。anchor归一化不因正文模式另行改变。
- 新增有界GPU工程核验入口：同一冻结SAM3的四项完整输出在零初始化时须全等，之后仅1–4步检查正文梯度、冻结参数与起止符保持；工程产物不可用作正式DDP恢复或初始化。
- 增加正文位置、错误模式/状态拒绝、CPU连续/恢复一致、评估/视频兼容及工程探针契约测试。是否提高分割质量须由独立同预算实验验证，不预设冻结起止符必然更好。

## 残差完整预算的显式续训（2026-09-12）

- 增加仓库外运行的残差预算扩展工具，只允许从已完成、完整验证、未触发早停且自身为最佳点的恢复状态增加epochs；不修改原trainer或冻结训练快照。
- 绑定实际训练清单、图像ID顺序及原始语义cache；只更新预算、配置指纹和派生planned进度，保留残差、AdamW、rank RNG/消费历史、验证历史和patience。普通resume仍拒绝直接改epochs。
- 增加边界拒绝、数据/cache和恢复状态篡改、原对象不变、保存重读、下一步优化器/RNG一致性及CLI拒绝覆盖测试。

## 冻结模型的MANO原生geometry诊断（2026-09-12）

- 新增有界、预登记的原SAM3 Detector点/框提示实验入口：投影MANO关节与网格范围，直接通过 `forward_grounding` 送入原geometry编码器；不训练、不接Tracker，不修改SAM3网络。
- 明确区分参考mask辅助估计的MANO网格框与直接读取的参考框；同几何切换左右文字，`visual`仅作为占位文本，不伪称无文本网络或独立盲测。
- 固定采样与最高分候选、实际输出Dice/边界/漏检，保存分开的RGB/参考/预测及原始提示位置，记录geometry实际接收的点数、框数和编码形状；遮挡点不根据参考mask筛除。
- 增加投影、无重复左镜像、坐标/框检查、实际空输出评分及Prompt正标签契约测试。

## 已完成预算的续训说明（2026-09-12）

- 空间预算扩展工具支持记录本次真实决策原因，不再硬编码“用户要求总十轮”；仅扩展已完成且验证过的epoch预算，保持学习率、优化器、RNG和严格恢复契约不变。
- 增加说明字段保留及空说明拒绝测试；不会修改正在运行的冻结代码。

## 分层验收：实际输出与视频系统评估（2026-09-12）

- 增加CPU连续视频分栏慢放：读取同协议已完成模型输出，RGB/辅助参考/每个模型mask分开，左右两行；固定播放速度不冒充原视频时标，用ffprobe核验全帧覆盖，MP4不参与量化评分。
- 新增Dex验证CPU逐帧诊断：全覆盖RLE Dice核验、检出/漏检转移、class与presence分数、逐序列统计，以及实际Dice的门控/候选形状描述性分解；不调整推理阈值，不把对侧重叠当已确认解剖学错误。
- 增加独立CPU验算入口：从保存的RLE和未变更参考PNG重算实际输出Dice/边界，校验帧、侧、质量标记、覆盖和哈希；不复用推理侧指标实现，不把验算通过当独立标注认证。
- 独立验算另核验固定阈值/边界协议、有限置信度、视频实际输出标记和运行期间输出文件不变；视频已接受mask不会被第二次按原始分数过滤，增加对应回归测试。

- 新评估协议v2以检测/系统实际输出的有手Dice、边界及逐侧空输出漏检/非空输出误报为主；候选Dice、含空帧Dice单列辅助。显式标记辅助参考和外部开发用途，不改旧发布与旧分数。
- 比较器核验实际输出指标并按总体/左右分别列出，输出RGB、参考和各模型实际mask分栏；禁止跨评估层/协议混表。
- 新增只读视频评估入口：预先指定连续录像或有界片段、双方独立会话、相同默认向前传播、完整帧覆盖、保留实例ID和输出概率。空间仅接detector mask head；残差对hand使用训练缓存，同时保留原VE的visual及其他占位文本。
- 连续空输出与ID集合变化仅作诊断；没有独立身份/接触标注时明确不报告IDSW或物体关系准确率。禁止把已查看的辅助参考改名为独立盲测。
- 增加实际输出/空参考、未知及问题标签排除、连续性、候选合并、视频占位文本保留和独立参考角色约束测试。

## 空间与文本残差统一评估及曲线（2026-09-12）

- 新增独立 `evaluate_hand_routes` 入口：原VE、空间适配器、输出残差复用相同RealSense评分、逐侧质量标记和预选可视化；默认固定128，显式支持全量。训练点必须指定完整且已验证的阶段epoch。
- 记录数据、代码、基础模型、精度、batch与阈值协议；比较器拒绝旧口径、协议不一致、记录哈希或覆盖错误，输出统一指标表和分开的RGB／参考／候选／阈值预测面板。不修改运行中的冻结训练快照。
- 曲线导出支持空间 `loss/mask_objective`，明确其仅优化focal＋Dice，与文本残差total loss不可直接比较；保留原始采样步与恢复段，不伪造epoch均值。
- 增加轮末验证约束、协议和记录校验、分栏输出、空间日志曲线回归测试。
...
- 新增回归测试：双卡父状态必须拒绝默认单进程规划，显式双卡规划通过。保留失败记录，重新启动前验证实际父checkpoint及数据契约。

## 空间适配器跨数据阶段续训（2026-09-12）

- 增加显式stage-source：完成且验证过的原阶段可转入新训练数据阶段；重置阶段采样步数，不重置AdamW计数、moment或rank RNG。
- checkpoint记录父权重SHA及优化器步数偏移；保留普通resume严格配置校验，禁止跨阶段更改验证集、学习率、batch/world size等非授权字段。
- 新阶段重新执行起点验证并独立选best；原阶段结果和冻结源码不修改。增加状态保留与非法迁移测试。
